            ...
            <div class="info">
              <el-form :model="formLogin" ref="vForm" :rules="rulesLogin" label-position="left" label-width="80px"
                ...
                <el-form-item>
                  ...
                  <el-button type="primary" @click="editDoctorLogin">确定</el-button>
                </el-form-item>
              </el-form>
            </div>
          </el-card>
            ...
            <el-form :model="formData" ref="vForm" :rules="rules" label-position="left" label-width="80px" size="small"
              ...
              <el-form-item label="头像" prop="avatar" class="label-center-align">
                ...
                <el-upload :file-list="avatarFileList" :headers="avatarUploadHeaders" :data="avatarUploadData"
                           action="http://localhost:8000/clinic/doctorInformationByDoctor/upload"
              ...
              <el-form-item>
                <el-button type="warning" @click="formData=doctorInformation">清除</el-button>
...
import {editDoctorInformation, editDoctorLogin, findSexCoding} from "../api/api";
...
export default {
  data() {
    ...
    return {
      ...
      doctorInformation: '',
      avatarImg: JSON.parse(localStorage.getItem("doctor")).avatar,
      imgSrc: '',
      cropImg: '',
      dialogVisible: false,
      cropper: null,
      name: JSON.parse(localStorage.getItem("doctor")).name,
      components: {},
      props: {},
      formData: {
        avatar: null,
  ...
  methods: {
    editDoctorLogin() {
      ElMessageBox.confirm(
          ...
          .then(() => {
            ...
            editDoctorLogin(para,configs).then((res) => {
      ...
      this.$refs['vForm'].validate(valid => {
        ElMessageBox.confirm(
            ...
            .then(() => {
              ...
              editDoctorInformation(this.formData,configs).then((res) => {
                if (res.data.msgId == 'C403') {
                  ElMessage({
                    type: 'error',
    ...
    this.formData = this.doctorInformation;
    let configs={
      headers: {
        token: sessionStorage.getItem('permissionToken')
      }
