  ...
  <section class="paybg">
    ...
    <div class="table-container">
      <table id="msg" class="msg">
        <tr>
          <td style="text-align: left">预约科室:&nbsp;&nbsp;{{ payList.personInformation.departmentName }}</td>
          <td style="text-align: left">预约时间:&nbsp;&nbsp;{{ payList.reserveTime }}</td>
        </tr>
        <tr>
          <td style="text-align: left">患者编号:&nbsp;&nbsp;{{ payList.appointmentInformation.patientid }}</td>
          <td style="text-align: left">医生编号:&nbsp;&nbsp; {{ payList.appointmentInformation.doctorid }}</td>
        </tr>
        <tr>
          <td style="text-align: left">预约患者:&nbsp;&nbsp;{{ payList.personInformation.patientName }}</td>
          <td style="text-align: left">预约医生:&nbsp;&nbsp; {{ payList.personInformation.doctorName }}</td>
        </tr>
        <tr>
          <td style="text-align: left">患者年龄:&nbsp;&nbsp;{{ payList.personInformation.patientAge }}</td>
          <td style="text-align: left">患者身份证号:&nbsp;&nbsp;{{ payList.personInformation.patientIdentityNumber }}</td>
        </tr>
      </table>
    </div>
    <div :hidden="hiddens">
...
<script>
import {getPayAppointmentInformation, getQRCode, paymentStatus} from "../../api/api";
    ...
    paymentStatus(){
      ...
      paymentStatus(this.payId+this.payList.appointmentInformation.patientid,configs).then((res)=>{
        if(res.data.msgId=='C201'){
          ...
          that.$router.push('/appointmentDoctor')
        }
      })
    },
    sendMsg() {
