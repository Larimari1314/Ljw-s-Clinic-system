    ...
    <div class="table-container">
      <table id="msg" class="msg">
        <tr>
          ...
          <td style="text-align: left">预约级别:&nbsp;&nbsp;{{ payList.doctorRegis }}</td>
        </tr>
        <tr>
          <td style="text-align: left">会诊金额:&nbsp;&nbsp;{{ payList.regisMoney }}</td>
          <td style="text-align: left">检测金额:&nbsp;&nbsp; {{ payList.medicalMoney }}</td>
        </tr>
        <tr>
          <td style="text-align: left">预约编号:&nbsp;&nbsp;{{ payList.queueNumber }}</td>
          <td style="text-align: left">预约医生:&nbsp;&nbsp; {{ payList.name }}</td>
        </tr>
        <tr>
          <td style="text-align: left">预约时间:&nbsp;&nbsp;{{ payList.reserveTime }}</td>
          <td style="text-align: left">确定预约日期:&nbsp;&nbsp;{{ payList.appointmentTime }}</td>
        </tr>
      </table>
      <h1>总金额：{{payList.totalMoney}}</h1>
    </div>
    <div :hidden="hiddens">
...
import {
  ...
  getQRCode, getQRCodeMedical,
  getQRCodeOrder,
  paymentStatus, queryPaymentInformation
} from "../../api/api";
...
export default {
  ...
  methods: {
    GetQRCode(){
      ...
      getQRCodeMedical(this.payId,configs).then((res)=>{
    ...
    paymentStatus(){
      ...
      paymentStatus(this.payId+this.payList.id,configs).then((res)=>{
        if(res.data.msgId=='C201'){
          ...
          that.$router.push('/medicalTechnologyOrder')
        }
      })
    },
    sendMsg() {
  ...
  mounted() {
    ...
    queryPaymentInformation(this.payId,configs).then((res)=>{
